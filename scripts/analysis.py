...
GROUP_BY: str | None = "coarse_selection_type"
...
def plot_spearman_vs_k(df: pd.DataFrame, group_by: str | None = GROUP_BY) -> None:
    """Plot mean assay-level Spearman vs k, one line per model, optionally grouped per `group_by`."""
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(6 * ncols, 4 * nrows), sharey=True, squeeze=False
    )
...
def plot_spearman_vs_k_per_assay(
    df: pd.DataFrame, assay_ids: list[str] | None = None
) -> None:
    """One panel per assay, one line per model. Pass `assay_ids` to plot a specific subset."""
    assays = sorted(assay_ids if assay_ids is not None else df["file_id"].unique())
    n = len(assays)
    ncols = min(4, n)
...
def rank_assays_by_lgbm_ridge_delta(
    ...
    pivot = pivot[
        pivot["lgbm"] > 0.15
    ]  # make sure the large delta is grounded in a reasonable fit
...
def print_summary_stats(df: pd.DataFrame) -> None:
    """Print statistics used in analysis."""
    # mean Spearman by model x k, averaged across assays
    by_k = (
    ...
    print("=== Mean Spearman by model x k (averaged across assays) ===")
    print(by_k.to_string())
    print()

    # mean Spearman by model x k x selection type
    by_sel = (
        df.groupby(["file_id", "model", "k", "coarse_selection_type"])["spearman"]
        .mean()
        .reset_index()
        .groupby(["model", "k", "coarse_selection_type"])["spearman"]
        .mean()
        .unstack("coarse_selection_type")
        .round(3)
    )
    print("=== Mean Spearman by model x k x selection type ===")
    print(by_sel.to_string())
    print()

    # full-data ceiling per selection type
    full = df[df["k"] == -1]
    ceiling = (
        full.groupby(["file_id", "model", "coarse_selection_type"])["spearman"]
        .mean()
        .reset_index()
        .groupby(["model", "coarse_selection_type"])["spearman"]
        .mean()
        .unstack("coarse_selection_type")
        .round(3)
    )
    print("=== Full-data (k=all) mean Spearman by model x selection type ===")
    print(ceiling.to_string())
    print()

    # fraction of full-data performance achieved at each k (lgbm)
    lgbm_full = by_k.loc[-1, "lgbm"]
    lgbm_k = by_k["lgbm"].drop(-1)
    frac = (lgbm_k / lgbm_full).round(3)
    print("=== LightGBM: fraction of full-data Spearman achieved at each k ===")
...
if __name__ == "__main__":
    ...
    # per-assay plot - pass assay_ids to limit to a specific subset
    assay_ids = [
        "TAT_HV1BR_Fernandes_2016",
        "A0A2Z5U3Z0_9INFA_Doud_2016",
        "TADBP_HUMAN_Bolognesi_2019",
    ]
    plot_spearman_vs_k_per_assay(df, assay_ids=assay_ids)
    ...
    rank_assays_by_lgbm_ridge_delta(
        df,
        top_n=10,
        output_path=Path("data/proteingym/results/tabdpt_target_assays.txt"),
    )
